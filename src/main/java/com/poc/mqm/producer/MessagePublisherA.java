...
@Component
@Slf4j
public class MessagePublisherA implements CommandLineRunner {

	@Autowired
	private JmsTemplate jmsTemplate;
	@Value("${publish.count:500}")
	private int publishCount;

	@Override
	public void run(String... args) throws Exception {
		log.info("MessagePublisherA started");
		IntStream.range(AppConstant.START_INDEX, publishCount).forEach(data -> {
			...
			jmsTemplate.convertAndSend(AppConstant.QUEUEA, customTextMessage, messagePostProcessor);
		});
		log.info("MessagePublisherA completed");
	}
